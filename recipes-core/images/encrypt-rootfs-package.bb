...
do_install() {
	rootfs_target=$(readlink -f ${DEPLOY_DIR_IMAGE}/core-image-minimal-imx6sxsabresd.tar.bz2)
   	...
   	tar -czf ${DEPLOY_DIR_IMAGE}/encrypted_rootfs_package.tar.gz -C ${DEPLOY_DIR_IMAGE} \
    	--transform "s/$(basename $rootfs_target)/core-image-minimal-imx6sxsabresd.tar.bz2/" \
    	--transform "s/$(basename $mfg_initramfs_target)/core-image-minimal-mfg-initramfs-imx6sxsabresd.cpio.gz/" \
    	--transform "s/$(basename $prod_initramfs_target)/core-image-minimal-prod-initramfs-imx6sxsabresd.cpio.gz/" \
    	$(basename $rootfs_target) $(basename $mfg_initramfs_target) $(basename $prod_initramfs_target)
